...
mod tests {
    ...
    fn assert_windows_path_eq_ignore_case(actual: &str, expected: &str) {
        assert!(
            actual.eq_ignore_ascii_case(expected),
            "expected Windows path {actual:?} to match {expected:?} ignoring ASCII case"
        );
    }
    ...
    fn launch_command_wraps_batch_files_with_system_cmd_exe() {
        ...
        let cmd_exe = launch.application_name.as_deref().unwrap();
        assert_windows_path_eq_ignore_case(cmd_exe, r"C:\Windows\System32\cmd.exe");
        assert_eq!(
            launch.command_line,
            format!(r#""{cmd_exe}" /d /c "C:\tools\npm.cmd" --version"#)
        );
    }
