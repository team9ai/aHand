...
/// Control-plane variant of [`maybe_inject_full_write_download_url`].
///
/// Worker tokens may be restricted to specific `device_ids`, so an
/// uploaded object key must remain under the authorized device's S3
/// prefix before the hub presigns a GET URL for the daemon.
pub(crate) async fn maybe_inject_full_write_download_url_for_device(
    state: &AppState,
    authorized_device_id: &str,
    request: &mut FileRequest,
) -> Result<(), ApiError> {
    if let Some(key) = full_write_s3_object_key(request) {
        validate_object_key_for_device(key, authorized_device_id)?;
    }
    maybe_inject_full_write_download_url(state, request).await
}

fn full_write_s3_object_key(request: &FileRequest) -> Option<&str> {
    use ahand_protocol::{file_request, file_write, full_write};

    let file_request::Operation::Write(write) = request.operation.as_ref()? else {
        return None;
    };
    let file_write::Method::FullWrite(fw) = write.method.as_ref()? else {
        return None;
    };
    let full_write::Source::S3ObjectKey(key) = fw.source.as_ref()? else {
        return None;
    };
    Some(key.as_str())
}
...
fn validate_object_key_for_device(key: &str, device_id: &str) -> Result<(), ApiError> {
    validate_object_key(key)?;
    let expected_prefix = format!("file-ops/{device_id}/");
    if !key.starts_with(&expected_prefix) {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "INVALID_S3_OBJECT_KEY",
            "FullWrite.s3_object_key is not scoped to the requested device",
        ));
    }
    Ok(())
}
