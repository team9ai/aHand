...
# Usage: upgrade.sh [--check] [--version v0.2.0]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --version)
      ...
      ;;
    *)
      echo "Usage: upgrade.sh [--check] [--version v0.2.0]"
      exit 1
      ;;
  esac
done
...
download "${RELEASE_URL}/checksums-rust.txt" "$TMP_DIR/checksums-rust.txt" 2>/dev/null || true

# Download binaries
echo "==> Downloading binaries..."
download "${RELEASE_URL}/ahandd-${SUFFIX}" "$TMP_DIR/ahandd"
download "${RELEASE_URL}/ahandctl-${SUFFIX}" "$TMP_DIR/ahandctl"

# Download admin SPA
echo "==> Downloading admin panel..."
download "${RELEASE_URL}/admin-spa.tar.gz" "$TMP_DIR/admin-spa.tar.gz"

# Download scripts
echo "==> Downloading scripts..."
download "${RELEASE_URL}/setup-browser.sh" "$TMP_DIR/setup-browser.sh" 2>/dev/null || true
download "${RELEASE_URL}/upgrade.sh" "$TMP_DIR/upgrade.sh" 2>/dev/null || true
...
if [ -f "$TMP_DIR/upgrade.sh" ]; then
  cp "$TMP_DIR/upgrade.sh" "$BIN_DIR/upgrade.sh"
  chmod +x "$BIN_DIR/upgrade.sh"
fi
