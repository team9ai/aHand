...
**Honest scope boundary (do NOT over-promise):** Windows has **no openat2/O_NOFOLLOW equivalent** — the TOCTOU race that `io_safe` closes on Unix CANNOT be fully closed on Windows. M4 does NOT claim TOCTOU parity. What M4 actually shipped: symlink create, ACL chmod, hardened env, policy case-folding, explicit pipe SD, and the Windows security regression suite — all to real parity. What M4 did **NOT** do (deferred, still backlog): post-open handle re-verification (`GetFinalPathNameByHandle` → re-check against policy) was **not implemented** — the residual TOCTOU window stays open, mitigated only by the documented single-tenant-host assumption. The Windows security tests annotate the unreachable openat2 analogues with `// no Windows equivalent: openat2 TOCTOU — single-tenant assumption (spec M4)`, matching the spec's TOCTOU note (§ M1/M4 backlog).
...
- [ ] Implementation via `icacls` subprocess (consistent with M1's `secure_file::restrict_to_owner` precedent — reuse that style) OR the `windows-sys` SetNamedSecurityInfo API; PREFER icacls for parity with existing code unless the proto demands fine-grained ACEs. Document the choice. **(IMPLEMENTED: deviated from the icacls preference — uses `SetNamedSecurityInfoW` + `PROTECTED_DACL_SECURITY_INFORMATION` for a true DACL replacement. `icacls /inheritance:r` cannot remove pre-existing _explicit_ ACEs, so it can't guarantee an owner-only DACL — see the `SECURITY-HIGH#1` comment in `fs_ops.rs`. Recursive apply self-walks and skips reparse points rather than `icacls /T`, which would follow junctions out of the allowlist — `SECURITY-HIGH#2`.)**
